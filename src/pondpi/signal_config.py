...
def load_signals(path, sensor_objects):
    ...
    sets a top-level `source:`, naming where its data comes from -- what
    that name refers to depends on the signal type's `reads_from_sensor`
    flag (see Signal.reads_from_sensor): a `reads_from_sensor` type
    (currently just `SensorSignal`) has `source:` name a configured
    sensor directly (validating that name, and holding onto the live
    object it names, is entirely that type's own responsibility, not
    something this module knows or checks); every other type has
    `source:` name another, earlier-defined signal instead, reading
    *that* signal's own `read()` output rather than a sensor's raw
    reading. This is how sequential composition (e.g. median-then-
    average) is expressed, without a dedicated "chain" type -- `source:`
    is simply every signal's one "where do I get data from" field,
    whichever kind of thing it names.
